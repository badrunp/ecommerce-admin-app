...
import { clearValidasiProduct, deleteHistoryProduct, deleteProduct, getAllProduct, updateGrapichProduct, updateProduct } from '../../actions'
...
function Product(props) {
    ...
    const [showModalProductDescription, setShowModalProductDescription] = useState(false)
    ...
    useEffect(() => {
        ...
        if(products.productHistory.length > 10){
            const id = []
            const removeHistory = products.productHistory.slice(10, products.productHistory.length);
    ...
    const handleDeleteProduct = (e, productId) => {
        ...
        // const dataArray = [];
        const productDetail = products.products.find((item, i) => item._id == productId)
        ...
        return (
            ...
            >
                <Form>
                    ...
                    <select className="form-control input-all mb-3" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>
                    ...
                    <textarea style={{height: "300px"}} className="form-control input-all" value={description} onChange={(e) => setDescription(e.target.value)} />

                    {
                        showProductImage.length > 0 &&
                        showProductImage.map((item, i) =>
                            <p className="render-image-name" key={i}>{item.image}</p>
                        )
                    }

                    {
                        ...
                        productPictures.map((item, i) =>
                            <p className="render-image-name" key={i}>{item.name}</p>
                        )
                    }
    
                        <div className="input-type-file mt-3">
                            <input type="file" name="image" id="productImage" onChange={handleChangeImage} />
                            <label htmlFor="productImage" className="label-for-file"><BiImageAdd className="input-file-icon" />Gambar</label>
                        </div>
                </Form>
    ...
    const renderModalProductImage = () => {
        return (
            ...
            <div className={darkMode ? "box-product-table-image bg-content-dark-mode" : "box-product-table-image"}>
            {
                ...
                productImage.map((img,i) => 
                    ...
                    <>
                        <p className="d-flex align-items-center"><RiFileWarningFill/> Tidak Ada Gambar</p>
                    </>
                )
            }
            </div>
    ...
    return (
        <Layout>
            {products.loading ? (<div className="loading-2"><ImSpinner9 className="loading-2-icon" /></div>) : null}
            <div className="container-layout product">
                <div className={darkMode ? "container-layout-header bg-content-dark-mode" : "container-layout-header"}>
                    <div className="container-layout-header-logo">
                        <div className={darkMode ? "bg-content-orange-dark-mode container-layout-header-icon " : "container-layout-header-icon "}>
                    ...
                    <div className="container-layout-header-action ">
                        <Link
                            ...
                            className={darkMode ? "container-layout-header-box-action container-layout-add bg-add-dark-mode" : "container-layout-header-box-action container-layout-add"}>
                            <RiAddLine className="container-layout-header-action-icon" />
                            <p style={{ padding: 0, margin: 0 }} className="container-layout-header-action-title">Tambah</p>
                        </Link>
                    </div>
                </div>
                ...
                <div className="product-main">
                    <div className="product-main-wrapper ">
                        <div className="product-main-left">
                            <div className={darkMode ? "product-main-left-quantity bg-content-dark-mode" : "product-main-left-quantity"}>
                                <div className="product-main-left-count">
                                    <div className={darkMode ? "product-count-circle border-color-orange-dark-mode" : "product-count-circle"}>
                                        <h1 className={darkMode ? "text-color-dark-mode" : "" }>{products.productsLength}</h1>
                                    </div>
                                    <h4 className={darkMode ? "text-color-dark-mode" : "" }>Jumlah Produk</h4>
                                </div>
                                <div className="product-count-top">
                                    ...
                                    <h4 className={darkMode ? "text-color-dark-mode" : ""}>Produk Terlaris</h4>
                                </div>
                            </div>

                            {
                                productsPriceMax(products.products).length > 0 ? (
                                    <div className={darkMode ? "product-main-left-grapich bg-content-dark-mode" : "product-main-left-grapich"}>
                                        <h4 style={{ marginBottom: "15px" }}>Harga Produk Termahal</h4>
                                        <LineChart
                                            className="grapich"
                                            width={530}
                                            height={250}
                                        ...
                                        >
                                            ...
                                            <Line type="monotone" dataKey="Harga" stroke={darkMode ? "#e16428" : "#0F66BD" } />
                                        </LineChart>
                                    </div>
                                ) : null
                            }
                        ...
                        <div className="product-main-right">
                            ...
                            <div className={darkMode ? "product-main-right-quatity-max bg-content-dark-mode" : "product-main-right-quatity-max"}>
                                {
                                    productsTotalMax(products.products).length > 0 ? (
                                        <>
                                            <h4 style={{ marginBottom: "15px" }}>Jumlah Produk Terbanyak</h4>
                                            <BarChart
                                                className="grapich"
                                                width={500}
                                                height={250}
                                                ...
                                                barSize={20}
                                            >
                                                <XAxis dataKey="Nama" scale="point" padding={{ left: 10, right: 10 }} />
                                                <YAxis />
                                                <Tooltip />
                                                <CartesianGrid strokeDasharray="3 3" />
                                                <Bar dataKey="Jumlah" fill={darkMode ? "#e16428" : "#0F66BD"} background={{ fill: '#eee' }} />
                                            </BarChart>
                                        </>
                                    ) : <Alert style={{ width: "100%" }} variant="danger"><VscWarning className="message-validasi-icon" /> Produk Kosong</Alert>
                                }
                            </div>

                            {
                                productsTotalLow(products.products).length > 0 ? (
                                    <div className={darkMode ? "product-main-right-quatity-low bg-content-dark-mode" : "product-main-right-quatity-low"}>
...

                {
                    products.products.length > 0 ? (
                        <div className={darkMode ? "table-data-product bg-content-dark-mode" : "table-data-product"}>
                            <div className="table-data-product-header">
                                ...
                                <Input
                                    ...
                                    className={darkMode ? "input-all-dark-mode input-search-product" : "input-all input-search-product"}
                            ...
                            <div className="category-pagination-main mt-3">
                    {
                                ...
                                <tbody>
                                    {
                                        ...
                                        products.productsS.map((item, i) =>
                                            <tr key={item._id} className={darkMode ? "link-color-dark-mode" : ""}>
                                                ...
                                                <td>{item.name}</td>
                                                <td>{item.slug}</td>
                                                <td><ImImages className="table-data-icon icon-table-image" onClick={(e) => handleModalProductImage(e, item._id)} /></td>
                                                <td>{item.quantity}</td>
                                                <td>Rp.{item.price}</td>
                                                <td><BsPencil className="table-data-icon icon-table-description" onClick={(e) => handleModalProductDescription(e, item._id)}/></td>
                                                <td>{item.category ? item.category.name : "Tidak Ada Kategori"}</td>
                                                <td className="td-action">
                                                    <button onClick={(e) => handleUpdateProduct(e, item._id)} className={darkMode ? "btn btn-sm container-layout-update btn-hover bg-update-dark-mode" : "btn btn-sm container-layout-update btn-hover"}><MdUpdate /> Ubah</button>
                                                    <button onClick={(e) => handleDeleteProduct(e, item._id, item.name)} className={darkMode ? "btn btn-sm container-layout-delete btn-hover bg-delete-dark-mode" : "btn btn-sm container-layout-delete btn-hover"}><RiDeleteBin6Fill /> Hapus</button>
                                                </td>
                                            </tr>
                                        )
                                    }
                        ...
                        <div className={darkMode ? "product-content-history bg-content-dark-mode" : "product-content-history"}>
                            <h3>History</h3>
...

            {
                products.message ? (
                    <div className="message-validasi-success message-validasi">
                        <div>
                            <p className="message-validasi-text"><BsCheckAll className="message-validasi-icon" /> {products.message}</p>
                        </div>
                    </div>
                ) : null
            }
